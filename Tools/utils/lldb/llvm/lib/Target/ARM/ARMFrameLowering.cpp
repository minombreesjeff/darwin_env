...
void ARMFrameLowering::emitPrologue(MachineFunction &MF) const {
  ...
  // Only emit CFI for iOS when it's not using Dwarf EH.
  const Function *Fn = MF.getFunction();
  MachineModuleInfo &MMI = MF.getMMI();
  const MCAsmInfo *AsmInfo = MF.getTarget().getMCAsmInfo();
  bool needsFrameMoves = STI.isTargetIOS() && Fn->needsUnwindTableEntry() &&
    AsmInfo->isExceptionHandlingDwarf();
  ...
  if (!AFI->hasStackFrame()) {
    ...
    if (needsFrameMoves) {
      ...
      BuildMI(MBB, MBBI, dl, TII.get(ARM::PROLOG_LABEL))
         .addSym(FrameLabel);
      MMI.addFrameInst(MCCFIInstruction::
                       createDefCfaOffset(FrameLabel, -NumBytes));
    }
    return;
  }
  ...
  if (needsFrameMoves) {
    ...
    //     sp = 1001c          ::  push {r4, r5, r6, r7, lr}
    //     fp = 10028          ::  add r7, sp, #12
    //     [sp] == 1001c       ::  push.w {r8, r10, r11}
    //     sp == 10010         ::    >>final value<<
    //
    // The frame pointer (r7) points to address 10028. The offset from r7 to the
    // initial sp is '8'. If we use an offset of '8' from 'r7', we get the CFI
    // offsets of -4 for lr, -12 for r6, -16 for r5, -20 for r4, -24 for r11, 
    // -28 for r10, and -32 for r8:
    //
    //  Ltmp1:
    //     .cfi_def_cfa r7, 8
    //  Ltmp2:
    ...
    // FIXME: This code is iOS specific.
    BuildMI(MBB, MBBI, dl, TII.get(ARM::PROLOG_LABEL))
      .addSym(FrameLabel)
      .setMIFlag(MachineInstr::FrameSetup);

    // Define the current CFA rule to use the provided FP.
    unsigned DwarfNum = RegInfo->getDwarfRegNum(FramePtr, true);
    // LR is stored in the highest address, followed by r7.
    MMI.addFrameInst(MCCFIInstruction::
                     createDefCfa(FrameLabel, DwarfNum, -8));

    for (unsigned i = 0, e = CSI.size(); i != e; ++i) {
      unsigned Reg = CSI[i].getReg();
      int FI = CSI[i].getFrameIdx();
      unsigned Offset = MFI->getObjectOffset(FI);
      DwarfNum = RegInfo->getDwarfRegNum(Reg, true);
      MMI.addFrameInst(MCCFIInstruction::
                       createOffset(FrameLabel, DwarfNum, Offset));
    }
  }
}
