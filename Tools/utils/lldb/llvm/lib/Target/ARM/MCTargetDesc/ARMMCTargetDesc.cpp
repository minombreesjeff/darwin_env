    ...
    if (SubVer == '8') {
      ...
      if (Len >= Idx+2 && TT[Idx+1] == 'm') {
        ...
        if (NoCPU)
          ...
          // Use CPU to figure out the exact features.
          ARMArchFeature = "+v7";
      } else if (Len >= Idx+2 && TT[Idx+1] == 'k') {
        if (NoCPU)
          // v7k: FeatureNEON, FeatureDB, FeatureDSPThumb2, FeatureHasRAS
          //      FeatureSlowFPBrcc,
          //      FeatureHasSlowFPVMLx,FeatureVMLxForwarding,
          //      FeatureT2XtPk, FeatureVFP4, FeatureMP,
          //      FeatureHWDiv, FeatureHWDivARM
          ARMArchFeature = "+v7,+neon,+db,+t2dsp,+ras,+slow-fp-brcc,"
            "+vmlx-forwarding,+t2xtpk,+vfp4,+mp,+hwdiv,+hwdiv-arm";
        else
          // Use CPU to figure out the exact features.
          ARMArchFeature = "+v7";
      } else {
...
static MCAsmInfo *createARMMCAsmInfo(const MCRegisterInfo &MRI, StringRef TT) {
  ...
  if (TheTriple.isOSDarwin())
    return new ARMMCAsmInfoDarwin(!TheTriple.getArchName().endswith("v7k"));
