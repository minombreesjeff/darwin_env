...
ARMTargetLowering::ARMTargetLowering(TargetMachine &TM)
  ...
  if (Subtarget->isTargetIOS()) {
    ...
    // Set the correct calling convention for APCS-VFP target, e.g. armv7k.
    if (Subtarget->isAPCS_VFP_ABI()) {
      for (int i = 0; i < RTLIB::UNKNOWN_LIBCALL; ++i)
        setLibcallCallingConv((RTLIB::Libcall)i, CallingConv::ARM_APCS_VFP);
    }
  }
