...
static bool ShouldDisableAutolink(const ArgList &Args,
  ...
  // Also support -fmodules-autolink, temporarily, but let -fautolink take
  // priority.
  Default = Args.hasFlag(options::OPT_fmodules_autolink,
...
void darwin::Link::AddLinkArgs(Compilation &C,
  ...
  // BEGIN APPLE: Copy -ffast-math settings for LTO. This is a temporary hack
  // (obviously!) that is not needed on trunk where we now have function
  // attributes to set these.
  if (D.IsUsingLTO(Args)) {
    ...
    // Handle various floating point optimization flags, mapping them to the
    // appropriate LLVM code generation flags. The pattern for all of these is
    // to default off the codegen optimizations, and if any flag enables them
    // and no flag disables them after the flag enabling them, enable the
    // codegen optimization. This is complicated by several "umbrella" flags.
    if (Arg *A = Args.getLastArg(options::OPT_ffast_math, FastMathAliasOption,
                                 options::OPT_fno_fast_math,
                                 options::OPT_ffinite_math_only,
                                 options::OPT_fno_finite_math_only,
      ...
      if (A->getOption().getID() != options::OPT_fno_fast_math &&
          ...
          A->getOption().getID() != options::OPT_fhonor_infinities) {
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back("-enable-no-infs-fp-math");
      }
    if (Arg *A = Args.getLastArg(options::OPT_ffast_math, FastMathAliasOption,
                                 options::OPT_fno_fast_math,
                                 options::OPT_ffinite_math_only,
      ...
      if (A->getOption().getID() != options::OPT_fno_fast_math &&
          ...
          A->getOption().getID() != options::OPT_fhonor_nans) {
        CmdArgs.push_back("-mllvm");
        CmdArgs.push_back("-enable-no-nans-fp-math");
      }
    ...
    if (!MathErrno && AssociativeMath && ReciprocalMath && !SignedZeros &&
        !TrappingMath) {
      CmdArgs.push_back("-mllvm");
      CmdArgs.push_back("-enable-unsafe-fp-math");
    }
    ...
    if (Arg *A = Args.getLastArg(options::OPT_ffast_math, FastMathAliasOption,
      ...
      if (A->getOption().getID() == options::OPT_ffp_contract) {
        ...
        if (Val == "fast" || Val == "on" || Val == "off") {
          CmdArgs.push_back("-mllvm");
          CmdArgs.push_back(Args.MakeArgString("-fp-contract=" + Val));
        } else {
          D.Diag(diag::err_drv_unsupported_option_argument)
            << A->getOption().getName() << Val;
        }
  ...
  // END APPLE: Copy -ffast-math settings for LTO.
