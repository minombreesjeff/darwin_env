...
static void handleAlignedAttr(Sema &S, Scope *scope, Decl *D, 
                              const AttributeList &Attr) {
  // check the attribute arguments.
  if (Attr.getNumArgs() > 1 || 
      (Attr.getNumArgs() == 1 && Attr.getParameterName())) {
    S.Diag(Attr.getLoc(), diag::err_attribute_wrong_number_arguments) << 1;
    return;
  }
  ...
  if (Attr.getNumArgs() == 1)
    ...
    CXXScopeSpec SS;
    UnqualifiedId Id;
    Id.setIdentifier(Attr.getParameterName(), Attr.getParameterLoc());
    ExprResult Result = S.ActOnIdExpression(scope, SS, SourceLocation(), Id,
                                            false, false);
    if (Result.isInvalid())
      return;
...
static void ProcessInheritableDeclAttr(Sema &S, Scope *scope, Decl *D,
  ...
  case AttributeList::AT_Aligned:     
    handleAlignedAttr(S, scope, D, Attr); 
    break;
