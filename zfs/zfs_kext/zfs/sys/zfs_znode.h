...
/*
 * The directory entry has the file type (currently unused on Solaris) in the
 * top 4 bits, and the object number in the low 48 bits (see DN_MAX_OBJECT_SHIFT).
 * The "middle" 12 bits are unused. Therefore, during zfs_readdir
 * we need to mask out the top 4 bits of the object number we retrieve from the 
 * zap to get the "pure" object id. Note that this is also used in zfs_zget
 * so we pass down the "pure" object id to the dmu in order to add holds to
 * the appropriate dnodes and dbufs.
 */
