...
zfs_link_create(zfs_dirlock_t *dl, znode_t *zp, dmu_tx_t *tx, int flag)
{
    ...
    /*
	 * Check the version.  We don't want to write out the dtype if this
	 * is an old pool where it's not supported
	 */	
	if (spa_version(dmu_objset_spa(zp->z_zfsvfs->z_os)) < ZFS_VERSION_BOOTFS) {
		error = zap_add(zp->z_zfsvfs->z_os, dzp->z_id, dl->dl_name,
	    		8, 1, &zp->z_id, tx);
	} else {
	   /*
	 	* We only use 48 bits for zid so use the top 4 bits(60-64) 
		* of the z_id to store the dtype for OSX 
	 	*/
		value = ZFS_DIRENT_MAKE(z_dtype, zp->z_id);
		error = zap_add(zp->z_zfsvfs->z_os, dzp->z_id, dl->dl_name,
			8, 1, &value, tx);
	}
	ASSERT(error == 0);
