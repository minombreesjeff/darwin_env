...
zfs_ioc_pool_log_history(zfs_cmd_t *zc)
{
	...
	if ((error = xcopyin(zc->zc_history, history_str, size)) != 0) {
		spa_close(spa, FTAG);
...
#endif
{
	...
	vec = ZFS_IOC_NUM(cmd);
	zc = (zfs_cmd_t *)data;
	zc->zc_cred = (uintptr_t)NOCRED;    /* wants vfs_context_current() */
	zc->zc_dev = dev;
	error = zfs_ioc_vec[vec].zvec_secpolicy(zc->zc_name, proc_ucred(p));
#else
	vec = cmd - ZFS_IOC;
