...
#pragma ident	"@(#)zfs_vnops.c	1.36	06/09/24 SMI"
...
int zfs_create_xattr(znode_t *, const char *, mode_t, cred_t *, znode_t **);
 ...
 */
static int
mappedwrite(struct vnode *vp, uint64_t woff, int nbytes, struct uio *uio, dmu_tx_t *tx)
{
	znode_t	*zp = VTOZ(vp);
	zfsvfs_t *zfsvfs = zp->z_zfsvfs;
	int64_t	start, off;
...
zfs_vnop_lookup(struct vnop_lookup_args *ap)
{
	...
	*vpp = NULL;
	...
	if (cnp->cn_namelen < sizeof(smallname)) {
		...
		MALLOC(filename, char *, cnp->cn_namelen, M_TEMP, M_WAITOK);
	}	
...
//		VNODE_ATTR_va_fsid |
	...
	if (VATTR_IS_ACTIVE(vap, va_data_alloc) || VATTR_IS_ACTIVE(vap, va_total_alloc)) {
		...
		vap->va_supported |= VNODE_ATTR_va_data_alloc | VNODE_ATTR_va_total_alloc;
	}

	if (VATTR_IS_ACTIVE(vap, va_name) && !vnode_isvroot(vp)) {
		if (zap_value_search(zfsvfs->z_os, pzp->zp_parent, zp->z_id, vap->va_name) == 0)
...
static int 
zfs_vnop_whiteout(__unused struct vnop_whiteout_args *ap)
{
	return (0);
}
...
#ifndef ZFS_READONLY
int
zfs_create_xattr(znode_t *dzp, const char *name, mode_t mode, cred_t  *cr, znode_t **zpp)
{
	znode_t  *xzp;
	zfsvfs_t  *zfsvfs = dzp->z_zfsvfs;
	zilog_t  *zilog = zfsvfs->z_log;
...
top:
	...
	if ( (error = zfs_dirent_lock(&dl, dzp, name, &xzp, 0)) ) {
		goto out;
	}
...
zfs_vnop_setxattr(struct vnop_setxattr_args *ap)
{
	...
	/* Lookup the attribute name. */
	error = zfs_dirlook(VTOZ(xdvp), (char *)ap->a_name, &xvp);
	...
	} else /* create the attr file. */ {
		znode_t  *xzp;

		error = zfs_create_xattr(VTOZ(xdvp), ap->a_name, VTOZ(vp)->z_phys->zp_mode, cr, &xzp);
		if (error)
			goto out;
