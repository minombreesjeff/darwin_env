...
int main(int argc, char * argv[]) 
{
   ...
   char* compileType = "";
    ...
    if (notAService)
    {
        ...
        ::StartServer(sXMLParser, &sMessagesSource, sPort, sStatsUpdateInterval, sInitialState, false); // No stats update interval for now
        ::RunServer();
        ::exit(0);
    }
...
void __stdcall ServiceMain(DWORD /*argc*/, LPTSTR *argv)
{
    ...
    if (::StartServer(sXMLParser, &sMessagesSource, sPort, sStatsUpdateInterval, sInitialState, false) != qtssFatalErrorState)
    {
