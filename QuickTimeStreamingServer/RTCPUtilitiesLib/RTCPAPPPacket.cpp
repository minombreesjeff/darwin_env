...
{
#define APPEND_TO_DUMP_ARRAY(f, v) {if (fDebug && mDumpArray) (void)sprintf(&mDumpArray[strlen(mDumpArray)], f, v);}
...
//      APPEND_TO_DUMP_ARRAY("vers=%u", itemVersion);
        ...
        switch (itemType)
        {
            ...
            case TW0_CHARS_TO_INT('o', 'b'): //'ob': // overbuffer window size
            {
                ...
                APPEND_TO_DUMP_ARRAY(", ovr_buffr_windw_siz=%u", fOverbufferWindowSize);
            }
            break;
