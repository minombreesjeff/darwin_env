...
{ 
    Assert(currentTimePtr);
    Assert(theStreamPtr);
    Assert(packetIDPtr);
    
    Bool16 packetReady = true;
    static const UInt32 sRTCPIntervalMSec = 5000;
    ...
    {  
        ...
        {    
           packetReady = false;
           *timeToSendThisPacketAgainPtr = sRTCPIntervalMSec - (*currentTimePtr - *lastPacketTransmitPtr);
           //printf("RTPSessionOutput::WritePacket Don't send RTCP packet id =%qu time diff last packet sent =%qd nexttime=%qd\n", *packetIDPtr,*currentTimePtr - *lastPacketTransmitPtr, *timeToSendThisPacketAgainPtr);
        }                
    }
