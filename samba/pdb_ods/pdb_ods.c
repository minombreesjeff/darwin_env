	...
	if (eDSNoErr == status) {
		if (eDSNoErr == status) {
			for (attributeIndex = 0, CFDictionaryGetKeysAndValues(samAttributes, (const void**)keys, (const void**)values); attributeIndex < count; attributeIndex++) {
				...
				if (isKey && isValue) {
					if (strcmp(key, kDSStdAuthSetPasswdAsRoot) == 0 || strcmp(key, kDSStdAuthSetNTHash) == 0 || strcmp(key, kDSStdAuthSetLMHash) == 0 ) {
...
//	pstrcpy(nt_username, username);
	...
	} else if (get_single_attribute(entry, kDS1AttrSMBRID, temp)) {
	...
	} else if (get_single_attribute(entry, kDS1AttrSMBGroupRID, temp)) {
...

{
	...
	if (need_ods_mod(pdb_add, sampass, PDB_GROUPSID)) {		
		...
		if(NULL != group_sid) {
			...
			sid_to_string(sid_string, group_sid);
			DEBUG(2, ("Setting Primary Group SID entry for user: %s [%s]\n", pdb_get_username(sampass), sid_string));
			make_a_mod(userEntry, kDS1AttrSMBSID, sid_string);
