		      ...
		      const char *fname,SMB_STRUCT_STAT *psbuf,int flags,mode_t mode, uint32 desired_access)
{
	...
	if (lp_BRLM())
	{
		...
		brlm_status = BRLMPosixOpen(fname, kBRLMDataFork, fsp->brlm_deny_mode | fsp->brlm_open_mode, local_flags /* creatPermissions */, mode, &brlm_ref, 0 /* sessionID */);
		fsp->brlm_ref = brlm_ref;
		if (BRLMNoErr == brlm_status)
			fsp->fd = BRLMGetDescriptor(brlm_ref);
		else 
			fsp->fd = -1;
		DEBUG(3,("open_file: [%d]BRLMPosixOpen file (%s) (brlm_perms=0x%X) (local_flags=0x%X) (mode=%d) (fd=%d) (brlm_ref=%X)\n", brlm_status, fname, fsp->brlm_deny_mode | fsp->brlm_open_mode, local_flags, mode, fsp->fd, brlm_ref));
	} else {
#endif
