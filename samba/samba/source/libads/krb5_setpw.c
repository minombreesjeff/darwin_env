...
ADS_STATUS sys_krb5_set_password(const char *kdc_host, const char *princ, const char *newpw, 
			     int time_offset)
{
				 ...
				 int time_offset)
{
	...
	return krb5_chg_password(kpasswd_server, target_principal,
    	...
    	return sys_krb5_set_password(kpasswd_server, target_principal,
				    ...
				    const char *password)
{
	...
	status = sys_krb5_set_password(ads->auth.kdc_server, principal, password, ads->auth.time_offset);
